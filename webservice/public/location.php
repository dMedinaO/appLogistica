...
try {
    ...
    $query = "SELECT * FROM cliente WHERE rutCliente = '".$data->rut."' LIMIT 0,1";
    $resultado = mysqli_query($conn, $query);

    if ( !$resultado ){
        http_response_code(404);
        echo json_encode(array(
            "message" => "El cliente con rut " . $data->rut . " no existe.",
            "data" => $data->rut,
        ));
        die();
    }
        ...
        while( $doc = mysqli_fetch_assoc($resultado) ){
            ...
            $query = "INSERT INTO geoPoint(latitude, longitude, createdPoint, modifiedPoint, rutCliente) VALUES" .
                "(".$data->latitude.",".$data->longitude.", NOW(), NOW(), '".$data->rut."')";
        ...
        echo json_encode(array(
            "message" => "La dirección fue almacenada."
        ));
    }
}
catch (Exception $e){
